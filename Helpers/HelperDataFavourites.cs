...
#region GeoSetter Favourites/Import
    ...
    /// <summary>
    ///     Reads the gs favs (fake) xml file
    /// </summary>
    ...
    internal static void ParseGeoSetterFavouritesXmlToFavourite(string fileNameToParse)
    {
        try
        {
            ...
            foreach (XElement item in doc.Descendants(name: "geosetterpositions").Elements())
            {
                GeoSetterFavourite geoSetterFavourite = new()
                {
                    ...
                    lat = double.TryParse(s: (string)item.Attribute(name: "lat"), result: out double lat) ? lat : 0,
                    lng = double.TryParse(s: (string)item.Attribute(name: "lng"), result: out double lng) ? lng : 0,
                    radius = int.TryParse(s: (string)item.Attribute(name: "radius"), result: out int radius)
                        ? radius
                        : 0,
                    autoassign = int.TryParse(s: (string)item.Attribute(name: "autoassign"), result: out int autoassign)
                        ? autoassign
                        : 0,
                    snap = int.TryParse(s: (string)item.Attribute(name: "snap"), result: out int snap) ? snap : 0,
                    alt = int.TryParse(s: (string)item.Attribute(name: "alt"), result: out int alt) ? alt : 0,
        ...
        catch
        {
            HelperControlAndMessageBoxCustomMessageBoxManager.ShowMessageBox(
                controlName: "mbx_FrmMainApp_ErrorImportGeoSetterFavourites", captionType: MessageBoxCaption.Error,
