              ...
              .WithParsed(action: o =>
               {
                   ...
                   {
                       // remove "\" from the end.
                       // trimend doesn't seem to work
                       string trimmedPath = o.Folder.EndsWith(value: "\"")
                           ? o.Folder.Substring(startIndex: 0, length: o.Folder.Length - 1)
                           : o.Folder;
                       if (Directory.Exists(path: trimmedPath))
                       {
                           FolderToLaunchIn = o.Folder;
                       }
                   }
               });
