...
using static GeoTagNinja.View.Forms.FrmMainApp;
...
public partial class FrmEditFileData : Form
{
    ...
    private void EnableDateControlsAndSetDropdownDefaultValues()
    {
        ...
        // load TZ-CBX
        foreach (string timezone in TimeZones.GetTimeZones())
        {
                ...
                try
                {
                    ...
                    else if (lstControlTypesWithDEValue.Contains(item: control.GetType()))
                    {
                        ...
                        if (dataInDirectoryElement != null)
                        {
                            ...
                            //    {
                            //        cbx_OffsetTime.Text =
                            //            TimeZones.GetFirstMatchingTzData(
                            //                offsetTimeToMatch:
                ...
                if (control is not NumericUpDown nud)
                {
                    if (control.Parent.Name == "gbx_TakenDate")
                    {
                        EnableSpecificControlAndDisableOthers(
                            ...
                            controlsToDisable: ControlTraversal
        ...
        List<Control> controlsToDisable)
    {
        IEnumerable<Control> controls =
            ControlTraversal.GetAllControls(control: parentControl);
        ...
        bool dirElementIsTheCurrentlySelectedDE)
    {
        ...
        else
        {
            ...
            // strCreateDate was rendered for display, so it is in the user's culture; asking for it back has to be
            // done in the same culture, which the old InvariantCulture argument here did not actually do (it was
            // applied to the ToString of an already-string value and so did nothing at all).
            _ = AttributeValueFormatter.TryParseDateTime(value: strCreateDate,
                context: ValueFormatContext.Display,
        ...
        try
        {
            locallyStoredToponomyDataForLatLng.Add(item: (
                ...
                $"{dtLocallyStoredToponomyData.Rows[index: 0][columnName: ToponomyColumns.DefaultEnglishNamesToColumnHeaders[ReadExifData.GetToponomyDataColumnName(DefaultColumnNamesFromElementAttributesForFileEditing.CountryCode, true)]]}"));
            locallyStoredToponomyDataForLatLng.Add(item: (
                ElementAttribute.Country,
                $"{dtLocallyStoredToponomyData.Rows[index: 0][columnName: ToponomyColumns.DefaultEnglishNamesToColumnHeaders[ReadExifData.GetToponomyDataColumnName(DefaultColumnNamesFromElementAttributesForFileEditing.Country, true)]]}"));
            locallyStoredToponomyDataForLatLng.Add(item: (
                ElementAttribute.City,
                $"{dtLocallyStoredToponomyData.Rows[index: 0][columnName: ToponomyColumns.DefaultEnglishNamesToColumnHeaders[ReadExifData.GetToponomyDataColumnName(DefaultColumnNamesFromElementAttributesForFileEditing.City, true)]]}"));
            locallyStoredToponomyDataForLatLng.Add(item: (
                ElementAttribute.State,
                $"{dtLocallyStoredToponomyData.Rows[index: 0][columnName: ToponomyColumns.DefaultEnglishNamesToColumnHeaders[ReadExifData.GetToponomyDataColumnName(DefaultColumnNamesFromElementAttributesForFileEditing.State, true)]]}"));
            locallyStoredToponomyDataForLatLng.Add(item: (
                ElementAttribute.Sublocation,
                $"{dtLocallyStoredToponomyData.Rows[index: 0][columnName: ToponomyColumns.DefaultEnglishNamesToColumnHeaders[ReadExifData.GetToponomyDataColumnName(DefaultColumnNamesFromElementAttributesForFileEditing.Sublocation, true)]]}"));
            locallyStoredToponomyDataForLatLng.Add(item: (
                ElementAttribute.GPSAltitude,
                $"{dtLocallyStoredToponomyData.Rows[index: 0][columnName: ToponomyColumns.DefaultEnglishNamesToColumnHeaders[ReadExifData.GetToponomyDataColumnName(DefaultColumnNamesFromElementAttributesForFileEditing.GPSAltitude, true)]]}"));

            TZ = $"{dtLocallyStoredToponomyData.Rows[index: 0][columnName: ToponomyColumns.DefaultEnglishNamesToColumnHeaders[ReadExifData.GetToponomyDataColumnName(DefaultColumnNamesFromElementAttributesForFileEditing.timezoneId, true)]]}";
        }
        ...
        if (dirElementIsTheCurrentlySelectedDE)
        {
            _ = AttributeValueFormatter.TryParseDateTime(value: tbx_CreateDate.Text,
                context: ValueFormatContext.Display,
                result: out createDate);
    ...
    {
        IEnumerable<Control> controlsInsideGbx_TakenDate =
            ControlTraversal.GetAllControls(control: gbx_TakenDate);
        ...
        EventArgs e)
    {
        ...
        IEnumerable<Control> controlsInsideGbx_CreateDate =
            ControlTraversal.GetAllControls(control: gbx_CreateDate);
