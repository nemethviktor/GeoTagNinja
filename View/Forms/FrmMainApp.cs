...
using GeoTagNinja.Helpers.Geo;
using GeoTagNinja.Helpers.Map;
using GeoTagNinja.Helpers.Startup;
using GeoTagNinja.Helpers.Text;
using GeoTagNinja.Helpers.UI;
using GeoTagNinja.Helpers.Update;
using GeoTagNinja.Model;
using GeoTagNinja.View.Dialogs;
...
public partial class FrmMainApp : Form
{
    ...
    /// <summary>
    ///     The running main window, or <see langword="null" /> when there is not one.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         Half the application reaches the main form by looking it up in
    ///         <see cref="Application.OpenForms" /> by name. That lookup was previously written out at every call site
    ///         as a hard cast, which has two failure modes: it returns <see langword="null" /> before the form is shown,
    ///         and at <b>design time</b> it throws outright.
    ///     </para>
    ///     <para>
    ///         The designer never instantiates the form being designed - it instantiates its base class,
    ///         <see cref="Form" />, and applies the designer surface to that. So while a form called "FrmMainApp" is
    ///         indeed in <see cref="Application.OpenForms" /> while you have it open in the designer, it is a plain
    ///         <see cref="Form" />, and casting it blew up any control whose constructor performed the lookup - taking
    ///         the whole design surface with it.
    ///     </para>
    ///     <para>
    ///         Using <c>as</c> here means design time and not-yet-shown both yield <see langword="null" /> instead of an
    ///         exception. Callers that run only while the app is up can keep dereferencing it directly; anything that
    ///         can run from a control constructor must null-check.
    ///     </para>
    /// </remarks>
    internal static FrmMainApp Instance => Application.OpenForms[name: "FrmMainApp"] as FrmMainApp;
    ...
    private async Task<Task> InitialiseApplication()
    {
        ...
        Task[] tasks =
        [
            SessionDataTables.GenericCreateDataTables(),
            AppStartup.AppStartupCreateDatabaseFile(),
            AppStartup.AppStartupWriteDefaultSettings(),
            AppStartup.AppStartupReadSQLiteTables(),
            AppStartup.AppStartupReadAppLanguage(),
            AppStartup.AppStartupReadCustomCityLogic(),
            AppStartup.AppStartupReadAPILanguage(),
            AppStartup.AppStartupApplyDefaults(
                settingTabPage: "tpg_Application",
                actuallyRunningAtStartup: true),
            LanguageTZ.DataReadCountryCodeDataFromWikipediaData(),
            AppStartup.AppStartupCheckWebView2(),
                                       ...
                                       EventArgs e)
    {
        ...
        if (!Program.CollectionModeEnabled)
        {
            AppStartup.AppSetupInitialiseStartupFolder(
        ...
        AppStartup.AppStartupLoadFavourites();
        AppStartup.AppStartupLoadCustomRules();
        AppStartupGetLastLatLngFromSettings();
        AppStartup.AppStartupGetOverwriteBlankToponomy();
        AppStartup.AppStartupGetToponomyRadiusAndMaxRows();
    ...
    private static void LaunchAutoUpdater()
    {
        ...
        AutoUpdater.ParseUpdateInfoEvent += AutoUpdaterCallbacks.OnParseUpdateInfo;
        AutoUpdater.CheckForUpdateEvent += AutoUpdaterCallbacks.OnCheckForUpdate;
                                                ...
                                                CoreWebView2WebMessageReceivedEventArgs e)
    {
        ...
        HelperVariables.HTMLDefaultLayer =
            MapLayers.GetMapLayers().FirstOrDefault(predicate: x => x.Value == layerName).Key ??
            HelperVariables.HTMLDefaultLayer;
                                            ...
                                            EventArgs e)
    {
                ...
                result: out double _))
        {
            ...
            {
                FileLocking.FileListBeingUpdated = true;
                foreach (ListViewItem lvi in lvw_FileList.SelectedItems)
                {
                    ...
                    if (dirElemFileToModify.Type == DirectoryElement.ElementType.File)
                    {
                        ...
                        while (FileLocking.GenericLockCheckLockFile(
                ...
                FileLocking.FileListBeingUpdated = false;
            }
        }
    ...
    internal void Request_Map_NavigateGo()
    {
        ...
        layerInfo = MapLayers.GetMapLayers()
        ...
        double? imgDirection)
    {
            ...
            && GPSLongitude != null)
        {
            ...
            (double, double) targetCoordinates =
                    GeoCalculations.CalculateTargetCoordinates(
                                              ...
                                              EventArgs e)
    {
        ...
        while (FileLocking.FileListBeingUpdated ||
               FileLocking.FilesAreBeingSaved)
        {
            await Task.Delay(millisecondsDelay: 10);
        }
                                          ...
                                          EventArgs e)
    {
        ...
        EditFileFormLauncher.ShowFrmEditFileData();
    }
                                                     ...
                                                     EventArgs e)
    {
        ...
        void ShowInvalidFolderMessage(string exceptionMessage = "")
        {
            ...
            {
                CurrentFoldersParent = StringOperations.Coalesce(
                    Directory.GetDirectoryRoot(path: CurrentFolder)
                  , "C:"
                );
            }
                                             ...
                                             EventArgs e)
    {
        ...
        if (HelperVariables.OperationChangeFolderIsOkay)
        {
            ...
            if (tbx_FolderName.Text.EndsWith(value: "\\"))
            {
                ...
                {
                    tmpStrParent = StringOperations.Coalesce(
                        Directory.GetDirectoryRoot(path: tbx_FolderName.Text)
                      , "C:"
                    );
                }

                string tmpStrRoot = StringOperations.Coalesce(
        Directory.GetDirectoryRoot(path: tbx_FolderName.Text),
        "C:"
    );
                tbx_FolderName.Text =
                    StringOperations.Coalesce(tmpStrParent, tmpStrRoot);
            }
        ...
        try
        {
            ...
            if (!RemoveGeoDataIsRunning)
            {
                ...
                foreach (ListViewItem lvi in lvw_FileList.SelectedItems)
                {
                    ...
                    FileLocking.GenericLockLockFile(
                        fileNameWithoutPath: dirElemFileToModify.ItemNameWithoutPath);
                    ...
                    FileLocking.GenericLockUnLockFile(
                               fileNameWithoutPath: dirElemFileToModify.ItemNameWithoutPath);
            ...
            if (dtToponomy.Rows.Count > 0)
            {
                ...
                List<(ElementAttribute attribute, string toponomyOverwriteVal)>
                    ...
                    [
                        (ElementAttribute.CountryCode, dtToponomy.Rows[index: 0][
                            columnName: ToponomyColumns.DefaultEnglishNamesToColumnHeaders[
                                ReadExifData.GetToponomyDataColumnName(DefaultColumnNamesFromElementAttributesForFileEditing.CountryCode, true)]].ToString()),
                        (ElementAttribute.Country, dtToponomy.Rows[index: 0][
                            columnName: ToponomyColumns.DefaultEnglishNamesToColumnHeaders[
                                ReadExifData.GetToponomyDataColumnName(DefaultColumnNamesFromElementAttributesForFileEditing.Country, true)]].ToString())
                    ];

                // there's only four in there.
                foreach (ElementAttribute attribute in CustomRulesMetadata.ToponomyReplaces())
                {
                ...
                string TZ = dtToponomy.Rows[index: 0][columnName:
                    ToponomyColumns.DefaultEnglishNamesToColumnHeaders[
                        ReadExifData.GetToponomyDataColumnName(DefaultColumnNamesFromElementAttributesForFileEditing.timezoneId, true)]].ToString();

                // The cell text was rendered for the user, so it has to be read back in their culture. The previous
                // InvariantCulture argument was applied to ToString of a string and therefore had no effect, while
                // the parse itself silently used the current culture anyway.
                _ = AttributeValueFormatter.TryParseDateTime(
                    value: lvi.SubItems[index: lvw_FileList
                                              .Columns[key: HelperVariables.COL_NAME_PREFIX +
            ...
            else
            {
                ...
                if (APIHandlingChoice.Contains(value: "yes"))
                {
                    ...
                    foreach (ElementAttribute attribute in
                             CustomRulesMetadata.ToponomyReplaces())
                    {
    ...
    private async Task lvw_FileList_LoadOrUpdate()
    {
        ...
        HelperVariables.LstTrackPath.Clear();
        FileLocking.FilesBeingProcessed.Clear();
        FileLocking.FileListBeingUpdated = true;
        RemoveGeoDataIsRunning = false;
